  ...
  return (
    <div className="h-[calc(100vh-12rem)] min-h-[600px] flex flex-col animate-in fade-in slide-in-from-bottom-4 duration-500 relative">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-8">
          <div>
              <h2 className="text-2xl font-bold text-white tracking-tight flex items-center gap-2">
                  <Users size={24} className="text-amber-400"/> People
              </h2>
          ...
          <div className="flex gap-3 w-full md:w-auto">
              <div className="relative flex-1 md:w-64">
                <input 
                    className="w-full bg-slate-900 border border-slate-700 rounded-none py-2.5 pl-9 pr-3 text-sm focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 transition-all outline-none text-slate-200 placeholder-slate-600"
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-1">
          {filtered.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-64 text-slate-500 opacity-60 border-2 border-dashed border-slate-800 rounded-none">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {filtered.map(contact => (
                      <div 
                          ...
                          className="bg-slate-900 border border-white/5 rounded-none p-6 hover:border-amber-500/50 hover:shadow-xl hover:shadow-amber-500/10 transition-all cursor-pointer group relative overflow-hidden"
                      >
                          {/* Top Decoration */}
                          <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-amber-500/50 to-violet-500/50 opacity-0 group-hover:opacity-100 transition-opacity"></div>

                          <div className="flex items-start justify-between mb-4">
                              <div className="w-12 h-12 rounded-none bg-slate-800 flex items-center justify-center text-lg font-bold text-amber-400 border border-white/5 shadow-inner">
                                  {contact.name.charAt(0).toUpperCase()}
                              </div>
                              <div className="p-2 rounded-none bg-white/5 text-slate-400 opacity-0 group-hover:opacity-100 transition-opacity">
                                  <Edit2 size={14} />
                              </div>
                          </div>
      ...
      {editingId && (
          <div className="absolute inset-0 z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
              ...
              <div className="bg-slate-900 border border-white/10 rounded-none shadow-2xl w-full max-w-lg relative z-10 animate-in zoom-in-95 duration-200 overflow-hidden">
                  ...
                  <form onSubmit={handleSave} className="p-6 space-y-4">
                      <div className="flex items-center gap-4 mb-6">
                           <div className="w-16 h-16 rounded-none bg-slate-800 flex items-center justify-center text-2xl font-bold text-amber-400 border border-white/10 shadow-inner">
                      ...
                      <div>
                          ...
                          <textarea 
                              className="w-full h-32 bg-slate-950/50 border border-slate-700/50 rounded-none p-4 text-sm text-slate-200 focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 outline-none resize-none placeholder-slate-600"
