...
const ShardRecovery: React.FC = () => {
    ...
    const handleRecover = async () => {
        try {
            ...
            // V4: Async Operation over Prime Field
    ...
    return (
        <div className="bg-slate-900/50 p-6 rounded-none border border-white/5 space-y-4">
            <div>
                ...
                <textarea 
                    className="w-full h-32 bg-slate-950 border border-slate-800 rounded-none p-4 font-mono text-xs text-slate-300 focus:ring-2 focus:ring-amber-500/50 outline-none resize-none custom-scrollbar"
            ...
            {error && (
                <div className="p-3 bg-red-500/10 border border-red-500/20 rounded-none text-red-400 text-xs font-bold flex items-center gap-2 animate-in fade-in">
                    <AlertTriangle size={14}/> {error}
                </div>
            )}
            
            {result && (
                <div className="p-4 bg-emerald-900/20 border border-emerald-500/30 rounded-none animate-in zoom-in-95">
                    <div className="text-xs text-emerald-500 font-bold uppercase tracking-wider mb-2">Recovered Secret</div>
                    <div className="flex items-center gap-2">
                        <div className="font-mono text-emerald-100 break-all bg-black/20 p-3 rounded-none flex-1 select-all border border-white/5">{result}</div>
                        <button onClick={() => navigator.clipboard.writeText(result)} className="p-2 hover:text-white text-emerald-500 transition-colors bg-emerald-500/10 rounded-none border border-emerald-500/20"><Copy size={16}/></button>
                    </div>
                </div>
            )}
        </div>
                        ...
                        <div className="flex flex-col md:flex-row gap-6 animate-in fade-in">
                            <div className="flex-1">
                                ...
                                <input 
                                    ...
                                    className="w-full h-1.5 bg-slate-700 rounded-none appearance-none cursor-pointer accent-amber-500"
                                />
                            </div>
                            <label className="flex items-center gap-3 cursor-pointer bg-slate-800/50 px-4 py-2 rounded-none border border-white/5 hover:bg-slate-800 transition-colors h-10 self-end">
                                <input 
                                    type="checkbox" 
                                    checked={editConfig.useSymbols} 
                                    onChange={e => setEditConfig({...editConfig, useSymbols: e.target.checked})}
                                    className="w-4 h-4 rounded-none bg-slate-700 border-slate-600 text-amber-500 focus:ring-amber-500"
                                />
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
             ...
             <div className="flex flex-wrap gap-2 w-full md:w-auto items-center">
                ...
                <div className="relative">
                    <Button 
                        ...
                        title="Sort Order"
                    >
                        <ListFilter size={18} className={sortMode !== 'alpha' ? "text-amber-400" : ""} />
                    </Button>
                    {isSortMenuOpen && (
                        <div className="absolute top-full right-0 mt-2 w-48 bg-slate-900 border border-white/10 rounded-none shadow-2xl z-50 p-1 flex flex-col gap-1 animate-in zoom-in-95 origin-top-right">
                            <button onClick={() => { setSortMode('alpha'); setIsSortMenuOpen(false); }} className={`flex items-center gap-2 px-3 py-2 rounded-none text-sm text-left ${sortMode === 'alpha' ? 'bg-amber-500/20 text-amber-400' : 'text-slate-400 hover:bg-slate-800'}`}>
                                <ListFilter size={14}/> Name (A-Z)
                            </button>
                            <button onClick={() => { setSortMode('created'); setIsSortMenuOpen(false); }} className={`flex items-center gap-2 px-3 py-2 rounded-none text-sm text-left ${sortMode === 'created' ? 'bg-amber-500/20 text-amber-400' : 'text-slate-400 hover:bg-slate-800'}`}>
                                <Calendar size={14}/> Newest First
                            </button>
                            <button onClick={() => { setSortMode('usage'); setIsSortMenuOpen(false); }} className={`flex items-center gap-2 px-3 py-2 rounded-none text-sm text-left ${sortMode === 'usage' ? 'bg-amber-500/20 text-amber-400' : 'text-slate-400 hover:bg-slate-800'}`}>
                                <BarChart2 size={14}/> Most Used
                            </button>
                            {onUpdateAllConfigs && (
                                <button onClick={() => { setSortMode('manual'); setIsSortMenuOpen(false); }} className={`flex items-center gap-2 px-3 py-2 rounded-none text-sm text-left ${sortMode === 'manual' ? 'bg-amber-500/20 text-amber-400' : 'text-slate-400 hover:bg-slate-800'}`}>
        ...
        {(isCommand || isFieldQuery) && (
            <div className="flex items-center gap-2 text-xs text-amber-300 bg-amber-500/10 px-3 py-2 rounded-none border border-amber-500/20 animate-in fade-in">
                <Terminal size={12} className="text-amber-400"/>
                <span>
                    {isCommand ? "Executing Command: " : "Field Filter: "} 
                    <strong>{search}</strong>
                </span>
                <span className="ml-auto text-amber-500/50 text-[10px] uppercase font-bold tracking-wider">
        ...
        {processedConfigs.length === 0 ? (
            <div className="text-center py-20 bg-slate-900/30 rounded-none border-2 border-dashed border-slate-800/50 flex flex-col items-center justify-center gap-4">
                <div className="w-16 h-16 bg-slate-800/50 rounded-none flex items-center justify-center text-slate-600">
                    <Wallet size={32} />
                </div>
                <div>
                ...
                {search && isModelReady() && !isExpanding && smartTerms.length === 0 && !isCommand && !isFieldQuery && (
                    <div className="mt-2 animate-in fade-in">
                        <button 
                            ...
                            className="text-xs flex items-center gap-2 text-amber-400 hover:text-amber-300 bg-amber-500/10 hover:bg-amber-500/20 px-4 py-2 rounded-none transition-all border border-amber-500/20"
                        >
                ...
                {isExpanding && (
                    <div className="flex items-center gap-2 text-xs text-amber-400 animate-pulse mt-2">
...
const VaultConfigCard: React.FC<{ 
    ...
    return (
        <div data-agent-id="vault-item-card" className={`border p-5 rounded-none transition-all duration-300 relative group overflow-hidden ${isCompromised ? 'bg-red-950/20 border-red-500/50 shadow-[0_0_20px_-5px_rgba(239,68,68,0.3)]' : 'bg-slate-900/80 border-white/5 hover:border-amber-500/30 hover:shadow-[0_4px_20px_-4px_rgba(0,0,0,0.5)]'}`}>
            ...
            <div className="bg-black/30 rounded-none p-3 flex items-center justify-between border border-white/5">
            ...
            <div className="flex justify-between items-center mt-3 text-[10px] uppercase font-bold tracking-widest text-slate-600">
                <div className="flex items-center gap-2">
                    <span className="bg-slate-800 px-2 py-0.5 rounded-none border border-white/5">v{config.version}</span>
                    {config.customPassword && <span className="bg-amber-900/50 text-amber-400 px-2 py-0.5 rounded-none border border-amber-500/20">CUSTOM</span>}
                </div>
                ...
                <div className="absolute inset-0 bg-slate-900/95 backdrop-blur-sm z-20 flex flex-col p-4 animate-in fade-in">
                    ...
                    <div className="flex-1 overflow-y-auto custom-scrollbar space-y-2 mb-2">
                        {generatedShards.map((shard, i) => (
                            <div key={i} className="bg-black/50 p-2 rounded-none border border-white/10 text-[10px] font-mono text-slate-300 break-all select-all hover:bg-black/80 transition-colors cursor-pointer" onClick={() => navigator.clipboard.writeText(shard)}>
                                {shard}
                            </div>
                        ))}
                    </div>
